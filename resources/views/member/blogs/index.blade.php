<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Pengaturan Blogify <a href="{{ route('member.blogs.create') }}" class="bg-blue-400 p-2 rounded-md text-white text-sm">Tambah Tulisan</a>
        </h2>
    </x-slot>
    <x-slot name="headerRight">
        <form action="{{ route('member.blogs.index') }}" method="get">
            <x-text-input id='search' name='search' type='text' class="p-1 m-0 md:w-72 w-80 mt-3 md:mt-0" value="{{ request('search') }}" placeholder='masukkan kata kunci ...'></x-text-input>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-6 py-4 text-center">
                                    ...
                                    <form class="inline" onsubmit="return confirm('Yakin akan menghaspus data ini')"  method="post" action="{{ route('member.blogs.destroy',['post'=>$value->id]) }}">
                                        @csrf
                                        @method('delete')
